...
RooRBWGaussConv::RooRBWGaussConv(const char *name, const char *title,
				 RooAbsReal& _x, RooAbsReal& _bwmean, RooAbsReal& _bwwidth,
				 RooAbsReal& _gmean, RooAbsReal& _gsigma, MassType _massType):
...
double RooRBWGaussConv::evalMass (double val) const {
  return D0MASS + val;
}
double RooRBWGaussConv::evalphsp(double val) const {
  ...
  if(evalP(realM0) == 0) return 0.0;

  double phspfactor = pow(evalP(realM)/evalP(realM0),3)
    * pow(evalbwff(evalP(realM))/evalbwff(evalP(realM0)),2) ;
...
Double_t RooRBWGaussConv::evalP (double mass) const {
  ...
  double lambda = (mass*mass-MPI*MPI-D0MASS*D0MASS)*(mass*mass-MPI*MPI-D0MASS*D0MASS)-4*MPI*MPI*D0MASS*D0MASS;
  if(lambda > 0) {return sqrt(lambda)/(2*mass);}
  else {return 0;}
}
